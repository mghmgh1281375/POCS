...
class Preprocess:
    def __init__(self, image, **kwargs):
        ...
        # cropping
        self.image = self.image[self.kwargs['vertical_crop']
                                [0]:self.kwargs['vertical_crop'][1], :]
    ...
    def find_rows(self):
        ...
        black_pixels = np.argwhere(
            project < project[int(self.image.shape[1]/2)])
        ...
        rows_boundries = np.reshape(
            black_pixel_boundries, (int(len(black_pixel_boundries)/2), 2))
        self.rows = np.absolute(
            np.mean(rows_boundries, axis=1, dtype=np.uint64))
        return self

    def find_cols_boundries(self):
        project = np.sum(self.image, axis=0)
        black_pixels = np.argwhere(
            project < project[int(self.image.shape[0]/2)])
        black_pixels = np.reshape(black_pixels, (black_pixels.shape[0],))
        black_pixel_boundries = []
        black_pixel_boundries.append(black_pixels[0])
        tmp = black_pixels[0]
